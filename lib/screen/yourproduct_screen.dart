...
const yourproductscreen({ Key? key }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    productwidget (_products[index].title,_products[index].id,_products[index].imageUrl,_products[index].price,_products[index].description,_products[index].status,
        ), 
    separatorBuilder: (context,index)=>Divider(thickness: 2,), itemCount:_products.length),
    ),
    );
